...
import it.isislab.dmason.util.SystemManagement.globals.Reducer;
...
public class Worker extends Observable
{
	...
	private boolean noStepLimit = true;
	...
	public void kill()
	{
		...
		{   
			...
			try {
				((DistributedState)state).closeConnectionJMS();
				...
				try {	
					while(!resetted)
					{
						while(!flag)
						{
							...
							if(totSteps != 0 || noStepLimit)
							{
								if(state.schedule.getSteps()-1 == totSteps && !noStepLimit)
								{
