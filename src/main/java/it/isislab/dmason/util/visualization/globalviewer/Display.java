...
public class Display extends GUIState
{
		...
		public void run()
		{
			...
			while(isActive)
			{
				if (isStarted) 
				{
					try 
					{	
						HashMap<String,Object> snaps = updates.getUpdates(step, numCells);
	...
	private List<SimComboEntry> simZoom;
	...
	private void imageViewMouseClicked(MouseEvent e) {
		...
		for(CellProperties cp : listCells){
				...
				if(cp.isMine(x, y))
				{
					...
					ImageIcon icon = new ImageIcon(ClassLoader.getSystemClassLoader().getResource("it.isislab.dmason/resource/image/zoomJOpt2.png"));
					...
					if(i==JOptionPane.YES_OPTION)
					{
						...
						{
							it.isislab.dmason.util.visualization.sim.app.DFlockers.FlockersWithUIView simulazione=new it.isislab.dmason.util.visualization.sim.app.DFlockers.FlockersWithUIView(new Object[]{connection,cp.id,true,numCells,width,height,fieldMode} );
							...
							{
								it.isislab.dmason.util.visualization.sim.app.DAntsForage.AntsForageWithUIZoom simulazione=new it.isislab.dmason.util.visualization.sim.app.DAntsForage.AntsForageWithUIZoom(new Object[]{connection,cp.id,true,numCells,width,height,fieldMode} );
								rZ=new RunnerZoom(simulazione, connection, cp.id, true,this,fieldMode,numCells,width,height,absolutePath,simulation);
							}
							else
								if(simulation.equals("it.isislab.dmason.util.visualization.sim.app.DParticles.Tutorial3ViewWithUI"))
								{
									it.isislab.dmason.util.visualization.sim.app.DParticles.Tutorial3ViewWithUI simulazione=new it.isislab.dmason.util.visualization.sim.app.DParticles.Tutorial3ViewWithUI(new Object[]{connection,cp.id,true,numCells,width,height,fieldMode} );
									rZ=new RunnerZoom(simulazione, connection, cp.id, true,this,fieldMode,numCells,width,height,absolutePath,simulation);
								}
						...
						if(i==JOptionPane.NO_OPTION)
						{
							...
							if(simulation.equals("it.isislab.dmason.util.visualization.sim.app.DFlockers.FlockersWithUIView"))
							{
								it.isislab.dmason.util.visualization.sim.app.DFlockers.FlockersWithUIView simulazione=new it.isislab.dmason.util.visualization.sim.app.DFlockers.FlockersWithUIView(new Object[]{connection,cp.id,false,numCells,width,height,fieldMode} );
								rZ=new RunnerZoom(simulazione, connection, cp.id, false,this,fieldMode,numCells,width,height,absolutePath,simulation);
							}
							else
								if(simulation.equals("it.isislab.dmason.util.visualization.sim.app.DAntsForage.AntsForageWithUIZoom"))
								{
									it.isislab.dmason.util.visualization.sim.app.DAntsForage.AntsForageWithUIZoom simulazione=new it.isislab.dmason.util.visualization.sim.app.DAntsForage.AntsForageWithUIZoom(new Object[]{connection,cp.id,false,numCells,width,height,fieldMode} );
									rZ=new RunnerZoom(simulazione, connection, cp.id, false,this,fieldMode,numCells,width,height,absolutePath,simulation);
								}
								else
									if(simulation.equals("it.isislab.dmason.util.visualization.sim.app.DParticles.Tutorial3ViewWithUI"))
									{
										it.isislab.dmason.util.visualization.sim.app.DParticles.Tutorial3ViewWithUI simulazione=new it.isislab.dmason.util.visualization.sim.app.DParticles.Tutorial3ViewWithUI(new Object[]{connection,cp.id,false,numCells,width,height,fieldMode} );
										rZ=new RunnerZoom(simulazione, connection, cp.id, false,this,fieldMode,numCells,width,height,absolutePath,simulation);
									}
					...
					break;
						...
						// TODO Auto-generated catch block
						e1.printStackTrace();
						JOptionPane.showMessageDialog(null, "Problem with simulation, impossible complete request! "+e1);
					}
			}
