 ...
 * </PRE>
 */
public class DDoubleGrid2DYLB extends DDoubleGrid2D implements DistributedField2DLB {
	...
	private DistributedRegionNumericLB<Integer,EntryNum<Double,Int2D>> region;
	...
	public synchronized boolean  synchro() {
			...
			if(balanceR<0){
				...
				for (int i = 0; i < height; i++) 
					for (int j = rmap.EAST_MINE.upl_xx; j <= rmap.EAST_MINE.down_xx; j++) 
						rmap.EAST_MINE.addEntryNum(new EntryNum<Double, Int2D>(
								field[j][i], new Int2D(j, i)));
			...
			if(balanceL<0){
				...
				for (int i = 0; i < height; i++) 
					for (int j = rmap.WEST_MINE.upl_xx; j <= rmap.WEST_MINE.down_xx; j++) 
						rmap.WEST_MINE.addEntryNum(new EntryNum<Double, Int2D>(
								field[j][i], new Int2D(j, i)));
		...
		if( rmap.WEST_OUT!=null )
		{
			...
			DistributedRegionNumericLB<Integer,EntryNum<Double,Int2D>> dr1=new DistributedRegionNumericLB<Integer,EntryNum<Double,Int2D>>(rmap.WEST_MINE.clone(),rmap.WEST_OUT.clone(),(sm.schedule.getSteps()-1),cellType,DistributedRegionNumericLB.LEFT,((DistributedField2DLB)((DistributedState)sm).getField()).getNumAgents(), getLeftMineSize(),my_width/*,cellsLeft*/);
			try 
			{	
		...
		if( rmap.EAST_OUT!=null )
		{
			DistributedRegionNumericLB<Integer,EntryNum<Double,Int2D>> dr2=new DistributedRegionNumericLB<Integer,EntryNum<Double,Int2D>>(rmap.EAST_MINE.clone(),rmap.EAST_OUT.clone(),(sm.schedule.getSteps()-1),cellType,DistributedRegionNumericLB.RIGHT,((DistributedField2DLB)((DistributedState)sm).getField()).getNumAgents(), getRightMineSize(),my_width/*,cellsRight*/);
			try 
			{			
		...
		try 
		{
			...
			while(!q.isEmpty())
			{
				region=(DistributedRegionNumericLB<Integer,EntryNum<Double,Int2D>>)q.poll();
				if(sm.schedule.getSteps()%1==0){
					if((sm.schedule.getSteps() % 2 == 0 && cellType.pos_j % 2 == 0)||(sm.schedule.getSteps() % 2 != 0 && cellType.pos_j % 2 != 0)){
						...
						if(region.type.pos_j == cellType.pos_j-1 && cellType.pos_j!=0){
							...
							//The balance can't be bigger than neighbor's width-AOI-1 otherwise the neighbor's region becames null
							if(balanceL>Math.min(region.width-MAX_DISTANCE-1,100000/(height*((DistributedMultiSchedule)(sm.schedule)).getNumFields())))
								balanceL=Math.min(region.width-MAX_DISTANCE-1,100000/(height*((DistributedMultiSchedule)(sm.schedule)).getNumFields()));
							//The balance can't be smaller than its width+AOI+1 otherwise this region becames null
							if(balanceL<Math.max(-my_width+MAX_DISTANCE+1,-100000/(height*((DistributedMultiSchedule)(sm.schedule)).getNumFields())))
								balanceL=Math.max(-my_width+MAX_DISTANCE+1,-100000/(height*((DistributedMultiSchedule)(sm.schedule)).getNumFields()));
							if(balanceL<0){
								//the region becames smaller
	 ...
	 */
	public void verifyUpdates(DistributedRegionNumericLB<Integer,EntryNum<Double,Int2D>> box)
	{
