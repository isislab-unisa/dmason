 ...
 *
 */
public class DDoubleGridNonUniform extends DDoubleGrid2D implements TraceableField{
	...
	public DDoubleGridNonUniform(int width, int height, SimState sm, int aoi, int id, int P, double initialGridValue, String name, String prefix) {
		...
		this.sm = sm;	
		this.AOI = aoi;
		this.updates_cache = new ArrayList<RegionNumeric<Integer,EntryNum<Double,Int2D>>>();
		this.name = name;
	...
	{		
		...
		rmap.NORTH_WEST_MINE=new RegionDoubleNumeric(own_x, own_y, own_x+AOI, own_y+AOI);
		...
		rmap.NORTH_EAST_MINE=new RegionDoubleNumeric(own_x+my_width-AOI, own_y, own_x+my_width, own_y+AOI);
		...
		rmap.SOUTH_WEST_MINE=new RegionDoubleNumeric(own_x, own_y+my_height-AOI,own_x+AOI, own_y+my_height);
		...
		rmap.SOUTH_EAST_MINE=new RegionDoubleNumeric(own_x+my_width-AOI, own_y+my_height-AOI,own_x+my_width,own_y+my_height);
		...
		rmap.WEST_MINE=new RegionDoubleNumeric(own_x,own_y,own_x + AOI , own_y+my_height);
		...
		rmap.EAST_MINE=new RegionDoubleNumeric(own_x + my_width - AOI,own_y,own_x +my_width , own_y+my_height);

		rmap.NORTH_MINE=new RegionDoubleNumeric(own_x ,own_y,own_x+my_width, own_y + AOI);
			...
			try
			{
				...
				if(returnValue!=null)
				{
					...
					if(name.contains("OUT"))
					{
						...
						{ 
							EntryNum<Double, Int2D> e = region.get(agent_id);
	...
	protected void publishRegions(Connection connWorker)
	{
		for(ORIENTATION ori:myCell.neighborhood.keySet())
		{
			...
			case NW:
				try 
				{
					DistributedRegionNumeric<Integer,EntryNum<Double,Int2D>> dr=new DistributedRegionNumeric<Integer,EntryNum<Double,Int2D>>(rmap.NORTH_WEST_MINE, rmap.NORTH_WEST_OUT,
			...
			case NE:
				try 
				{
					DistributedRegionNumeric<Integer,EntryNum<Double,Int2D>> dr=new DistributedRegionNumeric<Integer,EntryNum<Double,Int2D>>(rmap.NORTH_EAST_MINE,rmap.NORTH_EAST_OUT,
			...
			case SE:
				try 
				{
					DistributedRegionNumeric<Integer,EntryNum<Double,Int2D>> dr=new DistributedRegionNumeric<Integer,EntryNum<Double,Int2D>>(rmap.SOUTH_EAST_MINE,rmap.SOUTH_EAST_OUT,
			...
			case SW:
				try 
				{
					DistributedRegionNumeric<Integer,EntryNum<Double,Int2D>> dr=new DistributedRegionNumeric<Integer,EntryNum<Double,Int2D>>(rmap.SOUTH_WEST_MINE,rmap.SOUTH_WEST_OUT,
	 ...
	 */
	private void verifyUpdates(DistributedRegionNumeric<Integer,EntryNum<Double,Int2D>> box)
	{
		...
		{
			EntryNum<Double,Int2D> e_m = r_mine.get(agent_id);
		...
		RegionNumeric<Integer,EntryNum<Double,Int2D>> tomemorize=r_out.clone();
		tomemorize.clear();
		for(String agent_id : r_out.keySet())
		{
			EntryNum<Double,Int2D> e_m = r_out.get(agent_id);
	 ...
	 */
	private boolean setValue(double value, Int2D location){

		if(rmap.NORTH_WEST_MINE!=null && rmap.NORTH_WEST_MINE.isMine(location.x,location.y))
		{
			if(((DistributedMultiSchedule)sm.schedule).monitor.ZOOM)
				tmp_zoom.add(new EntryNum<Double, Int2D>(value, location));


			rmap.NORTH_WEST_MINE.addEntryNum(new EntryNum<Double,Int2D>(value, location));
			rmap.WEST_MINE.addEntryNum(new EntryNum<Double,Int2D>(value, location));
			myfield.addEntryNum(new EntryNum<Double,Int2D>(value, location));
			...
			if(rmap.NORTH_EAST_MINE!=null && rmap.NORTH_EAST_MINE.isMine(location.x,location.y))
			{
				...
				rmap.NORTH_EAST_MINE.addEntryNum(new EntryNum<Double,Int2D>(value, location));
				rmap.EAST_MINE.addEntryNum(new EntryNum<Double,Int2D>(value, location));
				myfield.addEntryNum(new EntryNum<Double,Int2D>(value, location));
				return rmap.NORTH_MINE.addEntryNum(new EntryNum<Double,Int2D>(value, location));
			}
			else
				if(rmap.SOUTH_WEST_MINE!=null && rmap.SOUTH_WEST_MINE.isMine(location.x,location.y))
				{
					...
					rmap.SOUTH_WEST_MINE.addEntryNum(new EntryNum<Double,Int2D>(value, location));
					rmap.WEST_MINE.addEntryNum(new EntryNum<Double,Int2D>(value, location));
					myfield.addEntryNum(new EntryNum<Double,Int2D>(value, location));
					...
					if(rmap.SOUTH_EAST_MINE!=null && rmap.SOUTH_EAST_MINE.isMine(location.x,location.y))
					{
						...
						rmap.SOUTH_EAST_MINE.addEntryNum(new EntryNum<Double,Int2D>(value, location));
						rmap.EAST_MINE.addEntryNum(new EntryNum<Double,Int2D>(value, location));
						myfield.addEntryNum(new EntryNum<Double,Int2D>(value, location));
						return rmap.SOUTH_MINE.addEntryNum(new EntryNum<Double,Int2D>(value, location));
					}
								...
								if(rmap.NORTH_MINE != null && rmap.NORTH_MINE.isMine(location.x,location.y))
								{
									...
									if(rmap.SOUTH_MINE != null && rmap.SOUTH_MINE.isMine(location.x,location.y))
									{
										...
										if(myfield.isMine(location.x,location.y))
										{
											...
											return myfield.addEntryNum(new EntryNum<Double,Int2D>(value, location));
										}
										else
											if(rmap.WEST_OUT!=null && rmap.WEST_OUT.isMine(location.x,location.y)) 
												return rmap.WEST_OUT.addEntryNum(new EntryNum<Double,Int2D>(value, location));
											else
												if(rmap.EAST_OUT!=null && rmap.EAST_OUT.isMine(location.x,location.y)) 
													return rmap.EAST_OUT.addEntryNum(new EntryNum<Double,Int2D>(value, location));
												else
													if(rmap.NORTH_OUT!=null && rmap.NORTH_OUT.isMine(location.x,location.y))
														return rmap.NORTH_OUT.addEntryNum(new EntryNum<Double,Int2D>(value, location));
													else
														if(rmap.SOUTH_OUT!=null && rmap.SOUTH_OUT.isMine(location.x,location.y))
															return rmap.SOUTH_OUT.addEntryNum(new EntryNum<Double,Int2D>(value, location));
														else
															if(rmap.NORTH_WEST_OUT!=null && rmap.NORTH_WEST_OUT.isMine(location.x,location.y)) 
																return rmap.NORTH_WEST_OUT.addEntryNum(new EntryNum<Double,Int2D>(value, location));
															else 
																if(rmap.SOUTH_WEST_OUT!=null && rmap.SOUTH_WEST_OUT.isMine(location.x,location.y)) 
																	return rmap.SOUTH_WEST_OUT.addEntryNum(new EntryNum<Double,Int2D>(value, location));
																else
																	if(rmap.NORTH_EAST_OUT!=null && rmap.NORTH_EAST_OUT.isMine(location.x,location.y)) 
																		return rmap.NORTH_EAST_OUT.addEntryNum(new EntryNum<Double,Int2D>(value, location));
																	else
																		if(rmap.SOUTH_EAST_OUT!=null && rmap.SOUTH_EAST_OUT.isMine(location.x,location.y))
																			return rmap.SOUTH_EAST_OUT.addEntryNum(new EntryNum<Double,Int2D>(value, location));
