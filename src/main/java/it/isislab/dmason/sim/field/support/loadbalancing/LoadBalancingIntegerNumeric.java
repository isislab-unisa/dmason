 ...
 */
public class LoadBalancingIntegerNumeric <E> implements LoadBalancingInterface{

	public ArrayList<MyCellInterface> list;
    /**
     *  void constructor
     */
	public LoadBalancingIntegerNumeric(){
	 ...
	 * @param topics         a hash of topics with a boolean value 
	...
	public boolean addForBalance(Object loc, Object element, Object myCell) {
		...
		MyCellIntegerNumeric m = (MyCellIntegerNumeric)myCell;
		int i = (Integer) element;
		Int2D location = (Int2D) loc;
		
		if(m.getMyField().isMine(location.x, location.y))
		{	
			return m.getMyField().addEntryNum(new EntryNum(i, location));
		}
		else
		{
			...
			for (int z = 0; z < fields.length; z++)
			{
				...
				try
				{
					...
					if(returnValue!=null)
					{
						...
						{   
							return region.addEntryNum(new EntryNum<Integer,Int2D>(i, location));
						}    
					}
