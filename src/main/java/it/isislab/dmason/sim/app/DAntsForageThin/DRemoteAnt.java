...
public class DRemoteAnt extends RemoteAnt<Int2D>
{
	...
	public void depositPheromone( final SimState state)
    {
		...
		Int2D location = af.buggrid.getObjectLocationThin(this);
		int x = location.x;
		int y = location.y;
		...
		if (DAntsForage.ALGORITHM == DAntsForage.ALGORITHM_VALUE_ITERATION)
		{
			...
			{
				double max = af.toFoodGrid.getThin(x, y);
				for(int dx = -1; dx < 2; dx++)
					for(int dy = -1; dy < 2; dy++)
					{
						...
						/*if((af.toFoodGrid.own_x<= x) && (x<(af.toFoodGrid.own_x+af.toFoodGrid.my_width)) && 
							...
							double m = af.toFoodGrid.getThin(_x, _y) * 
					...
					//}
				try {
					RemoteParam<Double> toSet=new RemoteParam<Double>(max);
					af.toFoodGrid.setDistributedObjectLocation(new Int2D(x, y),toSet, state);
			...
			{
				double max = af.toHomeGrid.getThin(x, y);
				for(int dx = -1; dx < 2; dx++)
					for(int dy = -1; dy < 2; dy++)
					{
						int _x = dx+x;
						...
						/*if((af.toHomeGrid.own_x<= x) && (x<(af.toHomeGrid.own_x+af.toHomeGrid.my_width)) && 
							...
							if (_x < 0 || _y < 0 || _x >= af.GRID_WIDTH || _y >= af.GRID_HEIGHT) continue;  // nothing to see here
							double m = af.toHomeGrid.getThin(_x, _y) * 
							(dx * dy != 0 ? // diagonal corners
									af.diagonalCutDown : af.updateCutDown) +
									reward;
							if (m > max) max = m;
				...
				try {
					RemoteParam<Double> toSet=new RemoteParam<Double>(max);
					af.toHomeGrid.setDistributedObjectLocation( new Int2D(x, y),toSet, state);
				} catch (DMasonException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
	...
	public void act( final SimState state )
	{
		...
		af.buggrid.getNeighborsMaxDistanceThin(x, y, af.buggrid.MAX_DISTANCE, false, new IntBag(), new IntBag());
		...
		if (hasFoodItem)  // follow home pheromone
		{
			...
			for(int dx = -1; dx < 2; dx++)
				for(int dy = -1; dy < 2; dy++)
				{
					...
					if ((dx == 0 && dy == 0) ||
							...
							af.obstacles.getThin(_x, _y) == 1) continue;  // nothing to see here
			...
			if (max == 0 && last != null)  // nowhere to go!  Maybe go straight
			{
				if (state.random.nextBoolean(af.momentumProbability))
				{
					int xm = x + (x - last.x);
					int ym = y + (y - last.y);
					if (xm >= 0 && xm < af.GRID_WIDTH && ym >= 0 && ym < af.GRID_HEIGHT && af.obstacles.getThin(xm, ym) == 0)
					{ max_x = xm; max_y = ym; }
				}
			}
