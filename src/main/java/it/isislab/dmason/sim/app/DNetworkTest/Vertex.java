 ...
 */
package it.isislab.dmason.sim.app.DNetworkTest;
...
public class Vertex extends RemoteVertex<Double2D> implements DVertexState{
	...
	public Vertex(){}
	public Vertex(String id,Integer community, Boolean isVisited, String label,Double2D pos) throws IOException
	{
		super.label=label;  
		super.id=id;
	...
	public Vertex(SimState state,Integer community, Boolean isVisited, String label,Double2D pos) throws IOException
	{
	...
	public void step(SimState state) {
		...
		out = ((Vertexes)state).network.getEdges(this, null);
		...
		{	
			...
			if (((Vertex)e.getFrom()).getVal((DistributedMultiSchedule) state.schedule)
					|| ((Vertex)e.getTo()).getVal((DistributedMultiSchedule) state.schedule)) {
				malati++;
			} else {
				sani++;
			}
	...
	public Object writeReplace() throws ObjectStreamException{
		...
		try {
			return new Vertex(id, community, isVisited, label, pos);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}return null;
