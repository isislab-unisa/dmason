...
import it.isislab.dmason.experimentals.util.management.master.thrower.DMasonMaster;
import it.isislab.dmason.experimentals.util.management.worker.PeerStatusInfo;
import it.isislab.dmason.experimentals.util.management.worker.StartUpData;
import it.isislab.dmason.experimentals.util.management.worker.UpdateData;
import it.isislab.dmason.experimentals.util.management.worker.Updater;
...
public class MasterDaemonStarter implements Observer
{
	...
	private DMasonMaster masterUi;

	/**
	 * Constructor.
	 * @param conn Connection with a provider.
	 */
	public MasterDaemonStarter(Connection conn, DMasonMaster ui)
	{
	...
	public void start(GeneralParam params, HashMap<String, EntryVal<Integer, Boolean>> config, String selSim, DMasonMaster gui,Address ftpAddress)
	{
		...
		if (fieldMode == DistributedField2D.HORIZONTAL_BALANCED_DISTRIBUTION_MODE)
		{
			int cnt = 0;
			...
			for (String workerTopic : config.keySet())
			{
				...
				for (int i = 0; i < fieldsInWorker; i++)
				{
					...
					//					if (i==0 /*&& i == Math.sqrt(numRegions) / 2*/) {
					//						data.setStep(true);
					//					}
					//if (cnt == numRegions / 2)
					if (cnt == 0)
		...
		else if(fieldMode == DistributedField2D.UNIFORM_PARTITIONING_MODE)
		{
			//One row and N columns || One column and N rows
			if(rows==1 || columns == 1){
				int cnt = 0;
				// Repeat for each worker
				for (String workerTopic : config.keySet())
			...
			else // N rows and N columns
			{
				...
				for (int i=0;i<rows;i++){
					for (int k=0;k<columns;k++){
						...
						//						data.setStep(true);
						//					}
						if (i==0 && k==0 /*&& i == Math.sqrt(numRegions) / 2*/) {
							data.setStep(true);
						}
	...
	private String createUploadDir(String selSim, Address ftpAddress) {
		...
		try {
			...
			//e.printStackTrace();
			System.out.println("Error FTP client not connected.");
		}
		return uploadDir;
	}
	...
	public void startBatch(GeneralParam params, HashMap<String, EntryVal<Integer, Boolean>> config, String selSim, List<EntryParam<String, Object>> simParam, Address ftpAddress, String topicPrefix, int testCounter)
	{
		...
		{
			if(rows==1 || columns == 1){ // One row (column) N columns (rows)
				int cnt = 0;
				// Repeat for each worker
				for (String workerTopic : config.keySet())
				{
