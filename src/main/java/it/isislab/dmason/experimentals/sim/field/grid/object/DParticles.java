...
public class DParticles extends DistributedState<Int2D> {
	...
	public void start()
    {
        ...
        try 
        {
        	...
        	trails = DObjectGrid2DFactory.createDObjectGrid2D(gridWidth, gridHeight,this,super.AOI,TYPE.pos_i,TYPE.pos_j,super.rows,super.columns,MODE,0,"trails",topicPrefix,false);
        ...
        Steppable decreaser = new Steppable()
        {
            ...
            {
                for (int j = 0; j < trails.field.length; j++) {
					for (int i = 0; i < trails.field[j].length; i++) {
						//Double ccc=new Double((Double) trails.field[i][j]);
						
								//ccc*=0.9;
								//trails.field[i][j]=ccc;
					}
				}
            	//trails.multiply(0.9);
