 ...
 *
 */
public class DRemoteAntNonUniformPartitioning extends RemoteAntNonUniformPartitioning<Int2D>
{
	/**
	 * 
	 */
	...
	double reward = 0;
	Int2D last;
	public DRemoteAntNonUniformPartitioning(){}
	public DRemoteAntNonUniformPartitioning(double initialReward){
	
		reward = initialReward; 
	}
    
	public DRemoteAntNonUniformPartitioning(DistributedState<Int2D> state, double initialReward) 
	{ 
		super(state);
		reward = initialReward; 
	}
	
	public DRemoteAntNonUniformPartitioning(DistributedState<Int2D> state, Int2D loc, double initialReward) 
	{ 
		super(state);
		this.pos = loc;
		reward = initialReward; 
    ...
    {
		final DAntsForageNonUniformPartitioning af = (DAntsForageNonUniformPartitioning)state;

		Int2D location = af.buggrid.getObjectLocation(this);
		int x = location.x;
		int y = location.y;
		
		if (DAntsForageNonUniformPartitioning.ALGORITHM == DAntsForageNonUniformPartitioning.ALGORITHM_VALUE_ITERATION)
		{
			// test all around
	...
	public void act( final SimState state )
	{
		...
		{
			double max = DAntsForageNonUniformPartitioning.IMPOSSIBLY_BAD_PHEROMONE;
			int max_x = x;
			int max_y = y;
			int count = 2;
			for(int dx = -1; dx < 2; dx++)
			...
			if (af.sites.field[max_x][max_y] == DAntsForageNonUniformPartitioning.HOME)  // reward me next time!  And change my status
		...
		else
		{
			...
			if (af.sites.field[max_x][max_y] == DAntsForageNonUniformPartitioning.FOOD)  // reward me next time!  And change my status
			{ reward = af.reward; hasFoodItem = ! hasFoodItem; 
