...
{	
	 ...
	 */
	public DSparseGrid2DYThin(int width, int height,int field_width,int field_height,SimState sm,int max_distance,int i,int j,int rows, int columns, String name,String prefix) 
	{		
		...
		for (int z = 0; z < fields.length; z++)
		{
		  ...
		  try
		  {
 			  ...
 			  if(returnValue!=null && name.contains("mine"))
 		      {
 				  ...
 				  if(region.isMine(location.x,location.y) && getObjectsAtLocation(location.x, location.y)==null)
		 	    	  return setObjectLocation(rm,new Int2D(location.x,location.y)); 
 		      }
	...
	{	
		...
		for(Region<Integer,Int2D> region : updates_cache)
			for(Entry<Int2D> e_m: region)
			{
				...
				rm.setPos(e_m.l);
				((DistributedState<Int2D>)sm).addToField(rm,new Int2D(e_m.l.x-own_x+2*MAX_DISTANCE, e_m.l.y));	
			}
	...
	public int getRightMineSize() {
	    ...
	    public int numObjectsAtLocationThin(final int x, final int y)
	        {
			MutableInt2D speedyMutableInt2D = new MutableInt2D(x-own_x+2*MAX_DISTANCE, y);
	    ...
	    public Double2D getObjectLocationAsDouble2DThin(Object obj)
	    	...
	    	return new Double2D(loc.x+own_x-2*MAX_DISTANCE,loc.y);
	        }

	    public Int2D getObjectLocationThin(Object obj)
	        {
	    	Int2D loc=(Int2D)super.getRawObjectLocation(obj);
			return new Int2D(loc.x+own_x-2*MAX_DISTANCE,loc.y);
	        }
	    ...
	    public boolean setObjectLocationThin(final Object obj, final int x, final int y)
	        {
	    	 Int2D loc=new Int2D(x-own_x+2*MAX_DISTANCE,y);
