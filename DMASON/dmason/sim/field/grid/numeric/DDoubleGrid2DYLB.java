...
import dmason.sim.field.DistributedRegionNumericLB;
 ...
 * </PRE>
 */
public class DDoubleGrid2DYLB extends DDoubleGrid2D {
	...
	public synchronized boolean  synchro() {
		...
		if(isLeft){ 
			...
			for (int i = 0; i < height; i++) {
				for (int j = rmap.right_out.upl_xx; j <= rmap.right_out.down_xx-MAX_DISTANCE; j++) {
					rmap.right_out.addEntryNum(new EntryNum<Double, Int2D>(
			...
			for (int i = 0; i < height; i++) 
				for (int j = rmap.right_mine.upl_xx; j <= rmap.right_mine.down_xx; j++) 
						rmap.right_mine.addEntryNum(new EntryNum<Double, Int2D>(
								field[j][i], new Int2D(j, i)));
			...
			for (int i = 0; i < height; i++) {
				for (int j = rmap.left_out.upl_xx+MAX_DISTANCE; j <= rmap.left_out.down_xx; j++) {
						rmap.left_out.addEntryNum(new EntryNum<Double, Int2D>(
							field[j][i], new Int2D(j, i)));
				}
			}
			...
			for (int i = 0; i < height; i++) 
				for (int j = rmap.left_mine.upl_xx; j <= rmap.left_mine.down_xx; j++) 
						rmap.left_mine.addEntryNum(new EntryNum<Double, Int2D>(
								field[j][i], new Int2D(j, i)));
		...
		if( rmap.left_out!=null )
		{
			...
			DistributedRegionNumericLB<Integer,EntryNum<Double,Int2D>> dr1=new DistributedRegionNumericLB<Integer,EntryNum<Double,Int2D>>(rmap.left_mine.clone(),rmap.left_out.clone(),(sm.schedule.getSteps()-1),cellType,DistributedRegionNumericLB.LEFT,((DistributedState)sm).getField().getNumAgents(),(int)((DistributedState)sm).getField().getLeftMineSize(),my_width/*,cellsLeft*/);
			try 
			{	
		...
		if( rmap.right_out!=null )
		{
			DistributedRegionNumericLB<Integer,EntryNum<Double,Int2D>> dr2=new DistributedRegionNumericLB<Integer,EntryNum<Double,Int2D>>(rmap.right_mine.clone(),rmap.right_out.clone(),(sm.schedule.getSteps()-1),cellType,DistributedRegionNumericLB.RIGHT,((DistributedState)sm).getField().getNumAgents(),(int)((DistributedState)sm).getField().getRightMineSize(),my_width/*,cellsRight*/);
			try 
			{			
