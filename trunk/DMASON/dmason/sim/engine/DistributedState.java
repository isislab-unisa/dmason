...
public abstract class DistributedState<E> extends SimState  
{
	 ...
	 public void init_connection()
	 {
		 ...
		 ThreadVisualizationCellMessageListener thread = 
			new ThreadVisualizationCellMessageListener((ConnectionNFieldsWithActiveMQAPI)connection, 
	 ...
	 private void connection_IS_toroidal(){
		 ...
		 if(MODE == 0){	//HORIZONTAL_MODE
			 ...
			 try {
				 ...
				 connection.createTopic(TYPE.pos_i+"-"+TYPE.pos_j+"L",((DistributedMultiSchedule)super.schedule).fields.size());
				 connection.createTopic(TYPE.pos_i+"-"+TYPE.pos_j+"R",((DistributedMultiSchedule)super.schedule).fields.size());
				 
				 connection.subscribeToTopic(TYPE.pos_i+"-"+((TYPE.pos_j-1+ NUMPEERS)%NUMPEERS)+"R");
				 connection.subscribeToTopic(TYPE.pos_i+"-"+((TYPE.pos_j+1+ NUMPEERS)%NUMPEERS)+"L");

				 UpdaterThreadForListener u1 = 
						 new UpdaterThreadForListener(connection, TYPE.pos_i+"-"+(((TYPE.pos_j-1+ NUMPEERS))%NUMPEERS)+"R", 
								 ((DistributedMultiSchedule)schedule).fields,listeners);
				 u1.start();
					
				 UpdaterThreadForListener u2 = 
						new UpdaterThreadForListener(connection, TYPE.pos_i+"-"+(((TYPE.pos_j+1+ NUMPEERS))%NUMPEERS)+"L",
							 ((DistributedMultiSchedule)schedule).fields,listeners);
			 	 u2.start();
			 } catch (Exception e) {
					// TODO Auto-generated catch block
			 ...
			 try {
					...
					UpdaterThreadForListener u1 = new UpdaterThreadForListener(connection,((i+sqrt)%sqrt)+"-"+((j+1+sqrt)%sqrt)+"L",((DistributedMultiSchedule)schedule).fields,listeners);
					u1.start();
					
					UpdaterThreadForListener u2 = new UpdaterThreadForListener(connection, ((i+sqrt)%sqrt)+"-"+((j-1+sqrt)%sqrt)+"R",((DistributedMultiSchedule)schedule).fields,listeners);
					u2.start();
					
					UpdaterThreadForListener u3 = new UpdaterThreadForListener(connection, ((i+1+sqrt)%sqrt)+"-"+((j+sqrt)%sqrt)+"U",((DistributedMultiSchedule)schedule).fields,listeners);
					u3.start();
					
					UpdaterThreadForListener u4 = new UpdaterThreadForListener(connection, ((i-1+sqrt)%sqrt)+"-"+((j+sqrt)%sqrt)+"D",((DistributedMultiSchedule)schedule).fields,listeners);
					u4.start();
					
					UpdaterThreadForListener u5 = new UpdaterThreadForListener(connection, ((i-1+sqrt)%sqrt)+"-"+((j-1+sqrt)%sqrt)+"CDDR",((DistributedMultiSchedule)schedule).fields,listeners);
					u5.start();
					
					UpdaterThreadForListener u6 = new UpdaterThreadForListener(connection, ((i-1+sqrt)%sqrt)+"-"+((j+1+sqrt)%sqrt)+"CDDL",((DistributedMultiSchedule)schedule).fields,listeners);
					u6.start();					
					
					UpdaterThreadForListener u7 = new UpdaterThreadForListener(connection, ((i+1+sqrt)%sqrt)+"-"+((j-1+sqrt)%sqrt)+"CUDR",((DistributedMultiSchedule)schedule).fields,listeners);
					u7.start();					
					
					UpdaterThreadForListener u8 = new UpdaterThreadForListener(connection, ((i+1+sqrt)%sqrt)+"-"+((j+1+sqrt)%sqrt)+"CUDL",((DistributedMultiSchedule)schedule).fields,listeners);
					u8.start();
	 ...
	 private void connection_NO_toroidal(){
		 ...
		 if(MODE == 0){	//HORIZONTAL_MODE
			 ...
			 try {
				 ...
				 connection.subscribeToTopic(TYPE.getNeighbourDiagLeftUp()+"CDDR");
				 UpdaterThreadForListener u1 = 
						new UpdaterThreadForListener(connection, TYPE.getNeighbourDiagLeftUp()+"CDDR",
								((DistributedMultiSchedule)schedule).fields,listeners);
				 u1.start();
					
				connection.subscribeToTopic(TYPE.getNeighbourDiagRightUp()+"CDDL");
				UpdaterThreadForListener u2 = 
						new UpdaterThreadForListener(connection, TYPE.getNeighbourDiagRightUp()+"CDDL",
								((DistributedMultiSchedule)schedule).fields,listeners);
				u2.start();
				
				connection.subscribeToTopic(TYPE.getNeighbourDiagLeftDown()+"CUDR");
				UpdaterThreadForListener u3 = 
						new UpdaterThreadForListener(connection, TYPE.getNeighbourDiagLeftDown()+"CUDR",
								 ((DistributedMultiSchedule)schedule).fields,listeners);
				u3.start();
				
				connection.subscribeToTopic(TYPE.getNeighbourDiagRightDown()+"CUDL");
				UpdaterThreadForListener u4 = 
						new UpdaterThreadForListener(connection, TYPE.getNeighbourDiagRightDown()+"CUDL",
								 ((DistributedMultiSchedule)schedule).fields,listeners);
				u4.start();
				...
				UpdaterThreadForListener u7 = 
						new UpdaterThreadForListener(connection,TYPE.getNeighbourUp()+"D",
								 ((DistributedMultiSchedule)schedule).fields,listeners);
				u7.start();
				...
				UpdaterThreadForListener u8 = 
						new UpdaterThreadForListener(connection, TYPE.getNeighbourDown()+"U",
								((DistributedMultiSchedule)schedule).fields,listeners);
				u8.start();
