...
import dmason.sim.field.continuous.DContinuous2DThin;
import sim.engine.*;
...
public class DFlocker extends RemoteFlock<Double2D> implements Orientable2D
{
    ...
    public Double2D consistency(Bag b, DContinuous2DThin flockers)
    {
        ...
        for(i=0;i<b.numObjs;i++)
        {
            ...
            if (!other.dead)
            {
                double dx = flockers.tdxThin(loc.x,other.loc.x);
                double dy = flockers.tdyThin(loc.y,other.loc.y);
	...
	{     
		...
		loc = flock.flockers.getObjectLocationThin(this);
