...
public class DDoubleGrid2DY extends DDoubleGrid2D {
	...
	public UpdaterNumericThreadForListener<Integer,EntryNum<Double,Int2D>> ut_left;
	public UpdaterNumericThreadForListener<Integer,EntryNum<Double,Int2D>> ut_right;
	 ...
	 */
	public DDoubleGrid2DY(int width, int height,SimState sm,int max_distance,int i,int j,int num_peers, String name, double initialGridValue, boolean fixed) {
	...
	private boolean createRegion()
	{
		...
		if(rmap.left_out == null)
		{
			...
			try 
			{
				connection.createTopic(cellType+"R"+NAME);//NAME distinguish a topic of other
		...
		if(rmap.right_out == null)
		{
			...
			try 
			{
				connection.createTopic(cellType+"L"+NAME);
		...
		{	// we create a thread for any side existent
			ut_right=new UpdaterNumericThreadForListener(connection, cellType,this.NUMPEERS,this,neighborhood,cellType.getNeighbourRight()+"L"+NAME,updates);
			ut_right.start();
		}
		
		if(rmap.left_out!=null)
		{
			ut_left=new UpdaterNumericThreadForListener(connection, cellType,this.NUMPEERS,this,neighborhood,cellType.getNeighbourLeft()+"R"+NAME,updates);
			ut_left.start();
		}
