		...
		throws DMasonException
	{
		if(MODE==HORIZONTAL_DISTRIBUTION_MODE)
		{
			if(width%num_peers == 0)
					return new DIntGrid2DY(width, height,sm, max_distance, i, j, num_peers, name, initialGridValue, fixed) ;
			else
			...
			if(MODE==SQUARE_DISTRIBUTION_MODE)
			{
				if((width% Math.sqrt(num_peers) == 0) && (height% Math.sqrt(num_peers) == 0))
					return new DIntGrid2DXY(width, height,sm, max_distance, i, j, num_peers, name, initialGridValue, fixed) ;
				else
