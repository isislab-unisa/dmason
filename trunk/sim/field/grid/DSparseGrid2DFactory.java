...
{	
	 ...
	 */
	public static final DSparseGrid2D createDSparseGrid2d(int width, int height,SimState sm,int max_distance,int i,int j,int num_peers,int MODE)
		throws DMasonException
	{
		if(MODE==HORIZONTAL_DISTRIBUTION_MODE)
		{
			if(width%num_peers == 0)
					return new DSparseGrid2DY(width, height,sm, max_distance, i, j, num_peers) ;
			else
			...
			if(MODE==SQUARE_DISTRIBUTION_MODE)
			{
				if((width% Math.sqrt(num_peers) == 0) && (height% Math.sqrt(num_peers) == 0))
					return new DSparseGrid2DXY(width, height,sm, max_distance, i, j, num_peers) ;
				else
