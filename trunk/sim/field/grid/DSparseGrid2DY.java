...
{	
	public UpdaterThreadForListener<Integer,Int2D> ut_left;
	public UpdaterThreadForListener<Integer,Int2D> ut_right;
	 ...
	 */
	public DSparseGrid2DY(int width, int height,SimState sm,int max_distance,int i,int j,int num_peers) 
	{		
		super(width, height);
	...
	private boolean createRegion()
	{
		...
		if(rmap.right_out!=null )
		{
			ut_right=new UpdaterThreadForListener(connection, cellType,this.NUMPEERS,this,neighborhood,cellType.getNeighbourRight()+"L",updates,listeners);
			ut_right.start();
		...
		if(rmap.left_out!=null)
		{
			ut_left=new UpdaterThreadForListener(connection, cellType,this.NUMPEERS,this,neighborhood,cellType.getNeighbourLeft()+"R",updates,listeners);
			ut_left.start();
			System.out.println(ut_left == null);
		}
	...
	{		
		...
		if( rmap.left_out!=null )
		{
			DistributedRegion<Integer,Int2D> dr1=new DistributedRegion<Integer,Int2D>(rmap.left_mine,rmap.left_out,(sm.schedule.getSteps()-1),cellType,DistributedRegion.LEFT);
			try 
			{	
		...
		if( rmap.right_out!=null )
		{
			DistributedRegion<Integer,Int2D> dr2=new DistributedRegion<Integer,Int2D>(rmap.right_mine,rmap.right_out,(sm.schedule.getSteps()-1),cellType,DistributedRegion.RIGHT);
			try 
			{			
	...
	public void setTable(HashMap table) {
		...
		System.out.println("Questa � la mia tabella!"+table);
	}
}
