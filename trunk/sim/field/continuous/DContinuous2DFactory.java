...
{	
	 ...
	 */
	public static final DContinuous2D createDContinuous2D(double discretization,double width, double height,SimState sm,int max_distance,int i,int j,int num_peers,int MODE)
	...
	{		
		if(MODE==HORIZONTAL_DISTRIBUTION_MODE)
		{
			...
			if(width%num_peers == 0 && (max_distance < my_width/2 ))
					return new DContinuous2DY(discretization,width, height,sm, max_distance, i, j, num_peers) ;
			else
			...
			if(MODE==SQUARE_DISTRIBUTION_MODE)
			{
				...
				if((width% Math.sqrt(num_peers) == 0) && (height% Math.sqrt(num_peers) == 0) && (max_distance < my_width/2) && (max_distance < my_height))
					return new DContinuous2DXY(discretization,width, height,sm, max_distance, i, j, num_peers) ;
				else
