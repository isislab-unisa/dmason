...
public class Worker{
	...
	public Worker(StartUpData data){
		super();
		this.data = data;
		bootstrap();
	}
	...
	public void bootstrap() {
			...
			step = data.isStep();
			gui = data.isGraphic();
			address = new Address((String)data.getParam()[0],(String)data.getParam()[1]);
			if(step)
			{
				try{
	...
	public void oneStep(){
		...
		if(step){
			try{
				if(gui)
					connection.publishToTopic(console.simulation.state.schedule.getSteps(), "step");
				else
					connection.publishToTopic(state.schedule.getSteps()-1,"step");
			}catch (Exception e) {
				e.printStackTrace();
			}
		   }	
	...
	{
		return state.getField().getLocalListener();
	}
